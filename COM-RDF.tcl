...
wm title $rdf "COM-RDF calculation"
...
proc frame_RDF  { fromatomlist toatomlist cutoff dr framenum} { 
# Frame for file
global rdfdata
global sets
...
# be resid
#set idkey "resid"
set fromindex [$fromatomlist get index]
set toindex [$toatomlist get index]


# Save all the center of mass in this frame
foreach fromid $fromindex {
	set fromresidue [atomselect top "same residue as index $fromid" frame $framenum]
	set fromcom($fromid) [measure center $fromresidue weight mass]
	$fromresidue delete
}

foreach toid $toindex {
	set toresidue [atomselect top "same residue as index $toid" frame $framenum]
	set tocom($toid) [measure center $toresidue weight mass]
	$toresidue delete
	}
puts "center of mass done"

foreach fromid $fromindex {
	puts "center of mass $fromcom($fromid)"	
	foreach toid $toindex {
		set distance [vecdist $fromcom($fromid) $tocom($toid)]
#to remove atoms from molecules
		if { [expr {$distance < $cutoff} ]&& [expr {$fromid != $toid}] } { 
...
$toatomlist frame $i
frame_RDF $fromatomlist $toatomlist $cutoff $dr  $i
