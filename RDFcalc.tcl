...
pack $rdf.boundary.l $rdf.boundary.l2 -side top -fill x
...
proc frame_RDF  { fromcoordlist fromidlist tocoordlist toidlist cutoff dr } { 
global rdfdata
global sets
...
set cutoff [$rdf.data.cutoff.num get]
...
global box_y
global box_z
set box_x [$rdf.boundary.x.num get]
set box_y [$rdf.boundary.y.num get]
set box_z [$rdf.boundary.z.num get]
set sets 0
...
#set molecule ID list to distinguish different molecules
set idkey "segname"
...
puts "Now Extracting coordinate information, please be patient "
#move the the right frame

$fromatomlist frame $i
$toatomlist frame $i

set fromcoordlist($sets) [$fromatomlist get { x y z }]
set tocoordlist($sets) [$toatomlist get { x y z }]

incr sets 1
}

#Delete all to accelerate the calculation
...
# frame_RDF $fromcoordlist $fromidlist $tocoordlist $toidlist $cutoff $dr
for { set i 0 } { $i <= $sets } { incr i 1 } {
set t1 [clock seconds]
foreach coordfrom $fromcoordlist($i) m $fromidlist {
        foreach coordto $tocoordlist($i) n $toidlist {
                if { $m == $n } continue
                set difference [vecsub $coordfrom $coordto]
